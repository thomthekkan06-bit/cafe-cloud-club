...
/* --- DYNAMIC MENU (Connected to Firebase) --- */
let menuData = []; 
// Make sure 'ref' and 'onValue' are imported at the top of your file!
// You already have them imported in your existing code, so this should work.
const menuRef = ref(db, 'menu');

onValue(menuRef, (snapshot) => {
    const data = snapshot.val();
    if (data) {
        // This handles if the data is an Array (imported via JSON) or Object
        menuData = Array.isArray(data) ? data : Object.values(data);
        renderMenu(); // Updates the screen instantly whenever you change stock
    } else {
...
function renderMenu() {
    ...
    filteredItems.forEach((item, index) => {
        ...
        // Check if the item is sold out in the database
const stockClass = item.isSoldOut ? " sold-out" : "";
card.className = `food-card ${item.type}${stockClass}`;
